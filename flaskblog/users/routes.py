...
@users.route('/register', methods=['GET', 'POST'])                        # formularz rejestracyjny
def register():
...
@users.route('/login', methods=['GET', 'POST'])                           # formularz logowania
def login():
    if current_user.is_authenticated:
        return redirect(url_for('main.home'))
    form = LoginForm()
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('Został wysłany email z instrukcją zresetowania hasła', 'info')
        return redirect(url_for('users.login'))
...
def reset_token(token):
    ...
    if user is None:
        flash('Token jest błędny lub wygasł', 'warning')
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Twoje hasło zostało zmienione! Możesz się zalogować', 'success')
        return redirect(url_for('users.login'))
